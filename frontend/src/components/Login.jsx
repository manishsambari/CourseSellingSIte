...
function Login() {
  ...
  return (
    <div className="bg-[#0a0a0f] text-[#e8e6f0] min-h-screen relative overflow-hidden flex flex-col justify-between selection:bg-purple-600 selection:text-white font-sans">
      ...
      <div className="fixed top-0 left-1/4 w-[500px] h-[500px] bg-purple-600/10 rounded-full blur-[140px] pointer-events-none -z-10 animate-glow" />
      <div className="fixed bottom-0 right-1/4 w-[500px] h-[500px] bg-blue-600/10 rounded-full blur-[140px] pointer-events-none -z-10 animate-glow" style={{ animationDelay: "2s" }} />
        ...
        <div className="glass-card rounded-3xl p-8 sm:p-10 border border-white/10 shadow-2xl space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-xs sm:text-sm text-gray-400">
              Enter your credentials to access your masterclasses
            </p>
          </div>

          {/* Role Switcher */}
          <div className="grid grid-cols-2 gap-1 p-1 rounded-2xl bg-white/5 border border-white/5 text-xs font-semibold">
            <button
              type="button"
              className="py-2 rounded-xl gradient-bg text-white shadow-sm"
            >
              Student Portal
            </button>
            <Link
              to="/admin/login"
              className="py-2 text-center rounded-xl text-gray-400 hover:text-white transition"
            >
              Admin Portal
            </Link>
          </div>
          ...
          <div className="pt-4 border-t border-white/5 text-center text-xs text-gray-400">
            ...
            <Link to="/signup" className="text-purple-400 hover:text-purple-300 font-semibold underline underline-offset-4">
              Sign up for free
            </Link>
          </div>
        </div>
      ...
      <footer className="p-6 text-center text-xs text-gray-500 relative z-10">
        &copy; {new Date().getFullYear()} CourseShip. Secure Learner Authentication.
      </footer>
    </div>
  );
}
